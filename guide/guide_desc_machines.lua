...
The Cooking Supplier is an on-demand supplier that cooks (smelts) items instantly, instead of waiting on a furnace's cook time. This convenience is paid for with Quantum Cycles, an abstract resource banked network-wide (see the Quantum Cycle Generator) rather than stored in the machine itself.
...
Each cook costs 1 Quantum Cycle per 0.1 seconds of that recipe's normal cook time, taken directly from the network's bank. If the network doesn't have enough Quantum Cycles banked, the Cooking Supplier simply can't fulfil the request - the same as if it were turned off. If a recipe's cook time is so long that it would never fit in the network's bank at all, the Cooking Supplier shows an error and won't accept it as a valid configuration.
